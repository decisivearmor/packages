...
data class PositionUpdateEvent (
...
) : PlatformVideoEvent()
 {
  ...
  override fun equals(other: Any?): Boolean {
    if (other !is PositionUpdateEvent) {
      return false
    }
    if (this === other) {
      return true
...
interface VideoPlayerInstanceApi {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: VideoPlayerInstanceApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.video_player_android.VideoPlayerInstanceApi.setNowPlayingMetadata$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.video_player_android.VideoPlayerInstanceApi.clearNowPlayingMetadata$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            val wrapped: List<Any?> = try {
