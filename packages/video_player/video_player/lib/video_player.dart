...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
  ...
  /// Creates a VideoPlayerController for Picture-in-Picture mode.
  ///
  /// This method checks if there's an active PiP player and reuses it if available.
  /// Otherwise, it creates a new controller using the standard network constructor.
  ///
  /// This is particularly useful when transitioning between videos while in PiP mode,
  /// as it ensures the PiP player is properly reused instead of creating a new instance.
  static Future<VideoPlayerController> createForPiP({
    ...
    // Check if there's an active PiP player
    final int? activePipId = await _videoPlayerPlatform.getActivePictureInPicturePlayerId();
    
    if (activePipId != null) {
      // Create a controller that reuses the existing player ID
      ...
      // Set the player ID directly to reuse the existing player
      controller._playerId = activePipId;
      
      // Initialize the controller with the existing player
      controller._creatingCompleter = Completer<void>();
      controller._creatingCompleter!.complete();
      
      // Get the current player value
      final Duration position = await controller.position ?? Duration.zero;
      final Duration duration = controller.value.duration;
      
      // Update the controller's value to reflect the current state
      ...
      controller._eventSubscription = _videoPlayerPlatform
          .videoEventsFor(activePipId)
          .listen(eventListener);
      
      return controller;
    }
    
    // No active PiP player, create a new controller normally
